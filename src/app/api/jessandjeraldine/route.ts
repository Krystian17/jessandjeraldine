import { emailClientTemplate } from "@/modules/jessandjeraldine/email/client";
import { emailGuestTemplate } from "@/modules/jessandjeraldine/email/guest";
import { RSVPDetailsType } from "@/modules/jessandjeraldine/reducer/RSVPInit";
...
export async function POST(request: Request) {
  ...
  const transporter = nodemailer.createTransport({
    service: "gmail",
    auth: {
      user: process.env.JJ_CLIENTID,
      pass: process.env.JJ_APIKEY,
    },
  });

  const clientname = "jessandjeraldine";
  const hostname = {
  ...
  const emailerName = {
    name: "Jess and Jeraldine Wedding <jessandjeraldinewedding@gmail.com>",
    address: "jeraldinequianzon@gmail.com",
  ...
  const SendSheet = () =>
    fetch("https://sheetdb.io/api/v1/8dyfpprqf3pdv", {
      method: "POST",
      headers: {
